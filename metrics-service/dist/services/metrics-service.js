...
class MetricsService {
    db = connection_1.DatabaseConnection.getInstance();
    /**
     * Record a single metric
     */
    async recordMetric(name, value, labels = {}, timestamp = new Date()) {
     ...
     */
    async getAggregatedMetrics(name, operation = 'avg', start, end) {
        if (!name || name.trim().length === 0) {
            throw new Error('Metric name cannot be empty');
        }
     ...
     */
    async cleanupOldMetrics(olderThan = '30 days') {
     ...
     */
    async getMetricsByLabels(labelFilters, options = {}) {
     ...
     */
    async getMetricNames(limit = 100) {
        const query = `
