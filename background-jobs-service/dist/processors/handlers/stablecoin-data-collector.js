...
exports.StablecoinDataCollector = void 0;
const base_handler_1 = require("./base-handler");
class StablecoinDataCollector extends base_handler_1.BaseHandler {
    constructor(config = {}) {
        super({
    ...
    async executeJob(job, logger) {
        ...
        // Validate sources
        const invalidSources = sources.filter((s) => !this.supportedSources.includes(s.toLowerCase()));
        if (invalidSources.length > 0) {
            logger.warn(`Unsupported sources ignored: ${invalidSources.join(', ')}`);
        }
        const validSources = sources.filter((s) => this.supportedSources.includes(s.toLowerCase()));
        ...
        const sourceOperations = validSources.map((source) => () => this.collectFromSource(source, ticker, logger));
        const collectionResults = await this.executeInParallel(sourceOperations, validSources);
    ...
    async collectFromSource(source, ticker, logger) {
        const timeoutMs = 60000; // 1 minute per source
        return this.withTimeout(this.performSourceCollection(source, ticker, logger), timeoutMs, `${source} data collection`);
    }
    async performSourceCollection(source, ticker, logger) {
    ...
    async collectFromCoinGecko(ticker, logger) {
    ...
    async collectTransparencyData(ticker, logger) {
    ...
    async collectDexData(ticker, logger) {
        ...
        return {
            ...
            weighted_average_price: pairs.reduce((sum, pair, _, arr) => sum + (pair.price * pair.liquidity_usd) / arr.reduce((total, p) => total + p.liquidity_usd, 0), 0),
            exchange_data: pairs,
            peg_stability: {
    ...
    async collectSocialData(ticker, logger) {
    ...
    async collectMetricsData(ticker, logger) {
    ...
    assessDataQuality(results) {
        ...
        const completeness = results.reduce((sum, result) => {
            const presentFields = expectedFields.filter(field => result && typeof result === 'object' && field in result).length;
...
exports.StablecoinDataCollector = StablecoinDataCollector;
//# sourceMappingURL=stablecoin-data-collector.js.map