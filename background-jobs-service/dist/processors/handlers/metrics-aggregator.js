...
class MetricsAggregator extends base_handler_1.BaseHandler {
    constructor(config = {}) {
        super({
    ...
    async executeJob(job, logger) {
        ...
        const { startTime, endTime, aggregationLevel, metrics = this.supportedMetrics, sources = ['api', 'database', 'cache', 'external'] } = job.data;
        ...
        const aggregationResults = await this.performAggregation(startTimestamp, endTimestamp, aggregationLevel, metrics, sources, logger);
    ...
    validateTimeRange(startTime, endTime, aggregationLevel) {
        ...
        const maxRange = maxRanges[aggregationLevel];
        if (diffMs > maxRange) {
            throw new Error(`Time range too large for ${aggregationLevel} aggregation. Max: ${maxRange / (24 * 60 * 60 * 1000)} days`);
        }
    }
    async performAggregation(startTime, endTime, aggregationLevel, metrics, sources, logger) {
        ...
        const metricOperations = metrics.map(metric => () => this.aggregateMetric(metric, startTime, endTime, aggregationLevel, sources, logger));
    ...
    async aggregateMetric(metricName, startTime, endTime, aggregationLevel, sources, logger) {
        logger.debug(`Aggregating metric: ${metricName}`);
        // Simulate data fetching from different sources
        const sourceDataPromises = sources.map(source => this.fetchMetricData(metricName, source, startTime, endTime, logger));
    ...
    async fetchMetricData(metricName, source, startTime, endTime, logger) {
    ...
    aggregateDataPoints(dataPoints, aggregationLevel) {
    ...
    calculateAggregatedMetric(metricName, dataPoints) {
    ...
    calculateDerivedMetrics(aggregatedMetrics, logger) {
        ...
        if (metricsMap.has('error_rate') && metricsMap.has('api_response_time')) {
            ...
            derivedMetrics.push({
                ...
                dataPoints: Math.min(metricsMap.get('error_rate').dataPoints, metricsMap.get('api_response_time').dataPoints),
                confidence: Math.min(metricsMap.get('error_rate').confidence, metricsMap.get('api_response_time').confidence)
            });
        }
        ...
        const performanceMetrics = [
            ...
            'job_processing_rate',
            'system_load'
        ].map(name => metricsMap.get(name)).filter(Boolean);
    ...
    generateSummaryStatistics(metrics) {
        return {
    ...
    // Helper methods
    getDataPointInterval(metricName) {
        const intervals = {
    ...
    generateRealisticMetricValue(metricName, source, timestamp) {
        const baseValues = {
    ...
    getBucketKey(timestamp, aggregationLevel) {
    ...
    parseBucketKey(bucketKey, aggregationLevel) {
    ...
    calculateBucketValue(points) {
        return points.reduce((sum, point) => sum + point.value, 0) / points.length;
    }
    getAggregationType(metricName) {
        const types = {
