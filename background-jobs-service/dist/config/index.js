...
const configSchema = joi_1.default.object({
    ...
    PORT: joi_1.default.number().integer().min(1).max(65535).default(3003),
    // Redis configuration
    REDIS_HOST: joi_1.default.string().default('localhost'),
    REDIS_PORT: joi_1.default.number().integer().min(1).max(65535).default(6379),
    REDIS_PASSWORD: joi_1.default.string().optional().allow(''),
    REDIS_DB: joi_1.default.number().integer().min(0).max(15).default(0),
    REDIS_KEY_PREFIX: joi_1.default.string().default('stablerisk:jobs:'),
    REDIS_RETRY_DELAY: joi_1.default.number().integer().min(100).max(30000).default(5000),
    REDIS_MAX_RETRIES: joi_1.default.number().integer().min(1).max(10).default(3),
    REDIS_CONNECT_TIMEOUT: joi_1.default.number().integer().min(1000).max(60000).default(10000),
    REDIS_COMMAND_TIMEOUT: joi_1.default.number().integer().min(1000).max(60000).default(5000),
    REDIS_KEEP_ALIVE: joi_1.default.number().integer().min(1000).max(300000).default(30000),
    ...
    DB_POOL_SIZE: joi_1.default.number().integer().min(1).max(50).default(10),
    DB_CONNECTION_TIMEOUT: joi_1.default.number().integer().min(1000).max(60000).default(30000),
    DB_QUERY_TIMEOUT: joi_1.default.number().integer().min(1000).max(300000).default(60000),
    // Job processor configuration
    MAX_WORKERS: joi_1.default.number().integer().min(1).max(50).default(5),
    POLLING_INTERVAL: joi_1.default.number().integer().min(100).max(60000).default(1000),
    STALE_JOB_TIMEOUT: joi_1.default.number().integer().min(60000).max(3600000).default(1800000), // 30 minutes
    MAX_CONCURRENT_JOBS: joi_1.default.number().integer().min(1).max(100).default(20),
    ...
    LOG_ENABLE_CONSOLE: joi_1.default.boolean().default(true),
    LOG_ENABLE_FILE: joi_1.default.boolean().default(false),
    LOG_FILENAME: joi_1.default.string().optional(),
    // Monitoring configuration
    ENABLE_METRICS: joi_1.default.boolean().default(true),
    METRICS_PORT: joi_1.default.number().integer().min(1).max(65535).default(9090),
    HEALTH_CHECK_INTERVAL: joi_1.default.number().integer().min(5000).max(300000).default(30000),
    // External service URLs (for job handlers)
    MAIN_APP_URL: joi_1.default.string().uri().default('http://localhost:3000'),
    METRICS_SERVICE_URL: joi_1.default.string().uri().default('http://localhost:3002'),
    // Job-specific configuration
    FIRECRAWL_API_KEY: joi_1.default.string().optional(),
    COINGECKO_API_KEY: joi_1.default.string().optional(),
    DEFAULT_JOB_TIMEOUT: joi_1.default.number().integer().min(10000).max(3600000).default(300000), // 5 minutes
    MAX_JOB_RETRIES: joi_1.default.number().integer().min(1).max(10).default(3),
    // Cleanup configuration
    CLEANUP_INTERVAL: joi_1.default.number().integer().min(60000).max(86400000).default(3600000), // 1 hour
    JOB_RESULT_MAX_AGE: joi_1.default.number().integer().min(86400000).max(2592000000).default(2592000000), // 30 days
    JOB_METRICS_MAX_AGE: joi_1.default.number().integer().min(604800000).max(7776000000).default(7776000000), // 90 days
}).unknown();
...
function getEnvironment() {
    return exports.configManager.getEnvironment();
}
function isDevelopment() {
    return exports.configManager.isDevelopment();
}
function isProduction() {
    return exports.configManager.isProduction();
}
function isTest() {
    return exports.configManager.isTest();
}
