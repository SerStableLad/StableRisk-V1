...
export class CacheInvalidator extends BaseHandler {
  private readonly maxKeysPerBatch = 50;
  private readonly maxPatternMatches = 1000;
  ...
  protected async executeJob(job: Job, logger: any): Promise<any> {
    ...
    const results = {
      invalidated_keys: [] as string[],
      failed_keys: [] as string[],
      errors: [] as string[],
      cascade_operations: [] as string[],
      performance: {
    ...
    if (pattern) {
      ...
      try {
        ...
        results.errors.push(`Pattern invalidation failed: ${(error as Error).message}`);
        results.performance.patternProcessingMs = Date.now() - patternStart;
      }
    }
    ...
    if (keys.length > 0) {
      ...
      try {
        ...
        results.errors.push(`Key invalidation failed: ${(error as Error).message}`);
        results.performance.keyProcessingMs = Date.now() - keyStart;
      }
    }
    ...
    if (cascade && results.invalidated_keys.length > 0) {
      ...
      try {
        const cascadeResults = await this.performCascadeInvalidation(
        ...
        results.errors.push(`Cascade invalidation failed: ${(error as Error).message}`);
        results.performance.cascadeProcessingMs = Date.now() - cascadeStart;
      }
    }
  ...
  private async invalidateByPattern(pattern: string, logger: any): Promise<{
    invalidated: string[];
    failed: string[];
    errors: string[];
  }> {
    ...
    try {
      ...
      for (const key of matchingKeys) {
        try {
          ...
          results.failed.push(key);
          results.errors.push(`Error invalidating ${key}: ${(error as Error).message}`);
        }
      }

    } catch (error) {
      results.errors.push(`Pattern matching failed: ${(error as Error).message}`);
    }

    return results;
  }

  private async invalidateSpecificKeys(keys: string[], logger: any): Promise<{
    invalidated: string[];
    failed: string[];
    errors: string[];
  }> {
        ...
        for (const cascadeKey of cascadeKeys) {
          try {
            ...
            logger.warn(`Cascade invalidation failed for ${cascadeKey}`, {
              metadata: { originalKey: key, error: (error as Error).message }
            });
          }
        }
      }
  ...
  private async invalidateSingleKey(key: string, logger: any): Promise<boolean> {
  ...
  private async determineCascadeKeys(originalKey: string): Promise<string[]> {
   ...
   * Sanitize cache keys
   */
  private sanitizeCacheKeys(keys: string[]): { valid: string[]; invalid: string[] } {
    const valid: string[] = [];
   ...
   */
  private estimateInvalidationImpact(keys: string[]): {
    estimatedAffectedSystems: string[];
