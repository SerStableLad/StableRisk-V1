  ...
  static routes(): Router {
    ...
    router.post('/set', async (req: Request, res: Response) => {
      try {
        ...
        const result = await CacheManager.getInstance().set(key, value, options);
    ...
    router.get('/get/:key', async (req: Request, res: Response) => {
      try {
        ...
        const value = await CacheManager.getInstance().get(decodedKey);
    ...
    router.post('/mget', async (req: Request, res: Response) => {
      try {
        const { keys } = req.body;
        ...
        const results = await CacheManager.getInstance().mget(keys);
    ...
    router.delete('/delete/:key', async (req: Request, res: Response) => {
      try {
        const { key } = req.params;
        const decodedKey = decodeURIComponent(key);
        const result = await CacheManager.getInstance().delete(decodedKey);
    ...
    // Invalidate by tag
    router.post('/invalidate/tag', async (req: Request, res: Response) => {
      try {
        const { tag } = req.body;
        ...
        const invalidatedKeys = await CacheManager.getInstance().invalidateByTag(tag);
    ...
    router.post('/invalidate/pattern', async (req: Request, res: Response) => {
      try {
        ...
        const invalidatedKeys = await CacheManager.getInstance().invalidateByPattern(pattern);
    ...
    router.get('/stats', async (req: Request, res: Response) => {
      try {
        const stats = await CacheManager.getInstance().getStats();
        res.json(stats);
      } catch (error: any) {
        res.status(500).json({ error: error.message });
      }
    ...
    // Bulk operations
    router.post('/bulk/set', async (req: Request, res: Response) => {
      try {
        ...
        const results = await Promise.allSettled(
          entries.map(entry => 
            CacheManager.getInstance().set(entry.key, entry.value, entry.options)
          )
        );
