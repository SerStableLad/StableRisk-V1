...
describe('CacheServiceClient Performance Tests', () => {
    ...
    describe('Operation Latency Benchmarks', () => {
        it('should meet get operation latency requirements', async () => {
            ...
            server.use(msw_1.http.get(`${baseUrl}/cache/get/perf-test-get`, () => {
        ...
        it('should meet mget operation latency requirements for bulk operations', async () => {
            ...
            server.use(msw_1.http.post(`${baseUrl}/cache/mget`, async ({ request }) => {
                ...
                const results = body.keys.map((key, index) => ({
                    key,
                    value: {
        ...
        it('should meet fallback cache performance requirements', async () => {
            ...
            server.use(msw_1.http.post(`${baseUrl}/cache/set`, () => msw_1.HttpResponse.error()), msw_1.http.get(`${baseUrl}/cache/get/*`, () => msw_1.HttpResponse.error()));
    ...
    describe('Throughput and Concurrency', () => {
        it('should handle high concurrent load efficiently', async () => {
            ...
            server.use(msw_1.http.post(`${baseUrl}/cache/set`, async () => {
                await (0, msw_1.delay)(Math.random() * 10); // Simulate some latency
                return msw_1.HttpResponse.json({ success: true });
            }));
        ...
        it('should maintain performance during mixed operation patterns', async () => {
            server.use(msw_1.http.post(`${baseUrl}/cache/set`, async () => {
                await (0, msw_1.delay)(5);
                return msw_1.HttpResponse.json({ success: true });
            }), msw_1.http.get(`${baseUrl}/cache/get/*`, async () => {
                await (0, msw_1.delay)(3);
                return msw_1.HttpResponse.json({
                    key: 'mixed-test',
                ...
                const body = await request.json();
                const results = body.keys.map((key) => ({ key, value: { data: key } }));
                return msw_1.HttpResponse.json({ results });
            }), msw_1.http.delete(`${baseUrl}/cache/delete/*`, async () => {
                await (0, msw_1.delay)(4);
                return msw_1.HttpResponse.json({ success: true });
            }));
        ...
        it('should scale fallback operations linearly', async () => {
            ...
            for (const size of testSizes) {
                ...
                const setPromises = Array.from({ length: size }, (_, i) => client.set(`scale-test-${i}`, { data: `value-${i}`, index: i }));
                await Promise.all(setPromises);
                const setTime = performance.now() - setStart;
                // Test get operations
                const getStart = performance.now();
                const getPromises = Array.from({ length: size }, (_, i) => client.get(`scale-test-${i}`));
    ...
    describe('Memory Usage and Efficiency', () => {
        ...
        it('should handle memory cleanup efficiently under pressure', async () => {
            server.use(msw_1.http.post(`${baseUrl}/cache/set`, () => msw_1.HttpResponse.error()));
    ...
    describe('Stress Testing', () => {
        it('should handle extreme load without degrading', async () => {
            ...
            server.use(msw_1.http.post(`${baseUrl}/cache/set`, async () => {
                ...
                await (0, msw_1.delay)(Math.random() * 5);
                return msw_1.HttpResponse.json({ success: true });
            }));
            const startTime = performance.now();
            const batchSize = 100;
        ...
        it('should recover quickly from service overload', async () => {
            ...
            const recoveryTest = async (phase, operations) => {
                const results = [];
                const start = performance.now();
        ...
        it('should maintain stability during prolonged operation', async () => {
            ...
            const operations = [];
            server.use(msw_1.http.post(`${baseUrl}/cache/set`, async () => {
                await (0, msw_1.delay)(Math.random() * 20);
                return msw_1.HttpResponse.json({ success: true });
            }), msw_1.http.get(`${baseUrl}/cache/get/*`, async () => {
                await (0, msw_1.delay)(Math.random() * 15);
                return msw_1.HttpResponse.json({
                    key: 'stability-test',
...
//# sourceMappingURL=cache-service-client.performance.test.js.map