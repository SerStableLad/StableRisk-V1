...
class MockMetricsCollector {
    ...
    recordCacheHit(key, dataSize, duration) {
...
const mockCompress = globals_1.jest.fn().mockImplementation((data) => {
    // Mock compression by just prefixing with 'compressed:'
    return Promise.resolve(`compressed:${data}`);
});
const mockDecompress = globals_1.jest.fn().mockImplementation((data) => {
        ...
        try {
            ...
            for (let i = 0; i < keys.length; i++) {
                ...
                if (valueResult && valueResult[1]) {
                    try {
                        const decompressedValue = await mockDecompress(valueResult[1]);
    ...
    describe('Core Functionality', () => {
        ...
        describe('Compression', () => {
            beforeEach(() => {
                ...
                cacheManager.config.enableCompression = true;
                cacheManager.config.compressionThreshold = 1024; // 1KB
            });
            ...
            test('should handle compression when disabled', async () => {
                cacheManager.config.enableCompression = false;
...
describe('Cache Manager Performance', () => {
    ...
    test('should handle concurrent operations efficiently', async () => {
        ...
        for (let i = 0; i < concurrency; i++) {
            operations.push(cacheManager.set(`concurrent:${i}`, { id: i, data: `test ${i}` }));
        }
        const start = Date.now();
