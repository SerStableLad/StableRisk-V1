...
class MockRedis {
    ...
    async zadd(key, score, member) {
        ...
        const index = members.findIndex((m) => m.member === member);
        if (index > -1) {
    ...
    async zrange(key, start, stop) {
        ...
        return members.slice(start, stop + 1).map((m) => m.member);
    }
    ...
    async zscore(key, member) {
        ...
        if (!existing)
            return null;
        const members = JSON.parse(existing);
        const found = members.find((m) => m.member === member);
...
class AccessPatternAnalyzer {
    ...
    async updatePattern(key, operation, dataSize) {
        ...
        const recentAccesses = history.filter(record => now - record.timestamp < oneHour);
    ...
    async getAccessHistory(key, limit = 50) {
        const history = this.accessHistory.get(key) || [];
        return history.slice(-limit).reverse(); // Return most recent first
    }
    async getPredictedNextAccess(key) {
    ...
    async analyzeAccessTrends(key) {
        ...
        const yesterday = history.filter(r => now - r.timestamp >= oneDay && now - r.timestamp < 2 * oneDay).length;
        const dayBefore = history.filter(r => now - r.timestamp >= 2 * oneDay && now - r.timestamp < 3 * oneDay).length;
        // Determine trend
        let trend = 'stable';
        let trendStrength = 0;
    ...
    async getOptimizationRecommendations() {
        ...
        const evictionCandidates = patterns
            .filter(([, pattern]) => pattern.frequency < 2 && pattern.importance < 0.3 && pattern.recency > 12)
            .map(([key]) => key);
        // TTL adjustments
...
describe('Access Pattern Analyzer', () => {
    ...
    describe('Error Handling', () => {
        test('should handle Redis errors gracefully', async () => {
            ...
            errorRedis.get = globals_1.jest.fn().mockRejectedValue(new Error('Redis error'));
        ...
        test('should handle extreme values', async () => {
            await expect(analyzer.recordRead('test', Number.MAX_SAFE_INTEGER)).resolves.not.toThrow();
            const pattern = await analyzer.getPattern('test');
            expect(pattern).toBeDefined();
        });
    });
});
//# sourceMappingURL=access-pattern-analyzer.test.js.map