...
class CacheInvalidationStrategy {
    ...
    async invalidateByPattern(pattern) {
        ...
        try {
            ...
            do {
                ...
                const [nextCursor, keys] = await this.redis.scan(parseInt(cursor), pattern, 100 // Batch size
                );
                cursor = nextCursor;
                if (keys.length === 0) {
                    continue;
    ...
    async invalidateByTTL(maxAge) {
        const cutoffTime = Date.now() - maxAge;
        const invalidatedKeys = [];
        // Scan all metadata keys to find expired entries
        let cursor = '0';
        do {
            const [nextCursor, keys] = await this.redis.scan(parseInt(cursor), 'cache:meta:*', 100);
            cursor = nextCursor;
    ...
    async invalidateSelective(selector) {
        ...
        if (selector.minAccessCount !== undefined || selector.maxAccessCount !== undefined) {
            const keys = await this.invalidateByAccessCount(selector.minAccessCount, selector.maxAccessCount);
            keys.forEach(key => allInvalidatedKeys.add(key));
    ...
    async scheduleInvalidation(delay, operation) {
    ...
    async findDependentKeys(dependency) {
        const dependentKeys = [];
        let cursor = '0';
        do {
            const [nextCursor, keys] = await this.redis.scan(parseInt(cursor), 'cache:meta:*', 100);
    ...
    async cleanupTagReferences(invalidatedKeys) {
    ...
    async invalidateByAccessCount(minCount, maxCount) {
        const invalidatedKeys = [];
        let cursor = '0';
        do {
            const [nextCursor, keys] = await this.redis.scan(parseInt(cursor), 'cache:meta:*', 100);
...
describe('Cache Invalidation Strategy', () => {
    ...
    describe('Scheduled Invalidation', () => {
        test('should schedule invalidation for future execution', async () => {
            ...
            const operation = globals_1.jest.fn().mockResolvedValue(['key1', 'key2']);
        ...
        test('should handle scheduled invalidation errors', async () => {
            const delay = 50;
            const operation = globals_1.jest.fn().mockRejectedValue(new Error('Scheduled error'));
            const taskId = await strategy.scheduleInvalidation(delay, operation);
    ...
    describe('Error Handling and Edge Cases', () => {
        test('should handle Redis connection errors gracefully', async () => {
            ...
            errorRedis.smembers = globals_1.jest.fn().mockRejectedValue(new Error('Redis error'));
...
//# sourceMappingURL=cache-invalidation-strategy.test.js.map