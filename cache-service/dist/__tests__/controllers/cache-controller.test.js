...
describe('Cache Controller API Endpoints', () => {
    ...
    beforeAll(() => {
        ...
        const MockedCacheManagerClass = cache_manager_1.CacheManager;
    ...
    describe('POST /cache/set', () => {
        ...
        it('should successfully set cache entry with all options', async () => {
            ...
            expect(mockCacheManager.set).toHaveBeenCalledWith('test:key', { data: 'test value', number: 123 }, {
                ttl: 3600,
                tags: ['test', 'example'],
                source: 'api-test',
                metadata: { version: '1.0' }
        ...
        it('should successfully set cache entry with minimal options', async () => {
            ...
            expect(mockCacheManager.set).toHaveBeenCalledWith('simple:key', 'simple string value', {});
        });
        it('should handle different data types', async () => {
            ...
            for (const testCase of testCases) {
                await (0, supertest_1.default)(app)
                    ...
                    .send(testCase)
                    .expect(201);
                expect(mockCacheManager.set).toHaveBeenCalledWith(testCase.key, testCase.value, {});
            }
        });
    ...
    describe('Concurrent Request Handling', () => {
        it('should handle concurrent GET requests', async () => {
            mockCacheManager.get.mockImplementation((key) => Promise.resolve(`value for ${key}`));
            const concurrentRequests = Array.from({ length: 100 }, (_, i) => (0, supertest_1.default)(app).get(`/cache/get/concurrent:${i}`));
        ...
        it('should handle concurrent SET requests', async () => {
            mockCacheManager.set.mockResolvedValue(true);
            const concurrentRequests = Array.from({ length: 50 }, (_, i) => (0, supertest_1.default)(app)
                .post('/cache/set')
        ...
        it('should handle mixed concurrent operations', async () => {
            ...
            mockCacheManager.delete.mockResolvedValue(true);
            const mixedRequests = [
                ...Array.from({ length: 20 }, (_, i) => (0, supertest_1.default)(app).get(`/cache/get/mixed:${i}`)),
                ...Array.from({ length: 20 }, (_, i) => (0, supertest_1.default)(app)
                    .post('/cache/set')
                    .send({ key: `mixed:set:${i}`, value: `value ${i}` })),
                ...Array.from({ length: 10 }, (_, i) => (0, supertest_1.default)(app).delete(`/cache/delete/mixed:del:${i}`))
            ];
    ...
    describe('Error Handling and Edge Cases', () => {
        ...
        it('should handle timeout scenarios', async () => {
            ...
            mockCacheManager.get.mockImplementation(() => new Promise(resolve => setTimeout(() => resolve('delayed value'), 100)));
            const response = await (0, supertest_1.default)(app)
...
//# sourceMappingURL=cache-controller.test.js.map