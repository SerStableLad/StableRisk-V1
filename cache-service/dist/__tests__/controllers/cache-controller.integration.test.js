...
describe('Cache Controller Integration Tests', () => {
    ...
    beforeAll(async () => {
        ...
        app.use('/cache', cache_controller_1.CacheController.routes());
        // Initialize cache manager
        cacheManager = cache_manager_1.CacheManager.getInstance();
    ...
    describe('API Endpoint Integration', () => {
        it('should handle complete cache lifecycle through API', async () => {
            ...
            // 1. Set cache entry
            const setResponse = await (0, supertest_1.default)(app)
                .post('/cache/set')
                .send({
                key: testKey,
                value: testValue,
        ...
        it('should handle batch operations correctly', async () => {
            ...
            // Bulk set
            const bulkSetResponse = await (0, supertest_1.default)(app)
                .post('/cache/bulk/set')
                .send({ entries })
                .expect(200);
            ...
            // Multi-get
            const mgetResponse = await (0, supertest_1.default)(app)
                .post('/cache/mget')
        ...
        it('should handle tag-based invalidation', async () => {
            ...
            await (0, supertest_1.default)(app)
                ...
                .expect(200);
            // Invalidate by tag
            const invalidateResponse = await (0, supertest_1.default)(app)
    ...
    describe('Performance and Load Testing', () => {
        it('should handle concurrent requests', async () => {
            ...
            const setPromises = Array.from({ length: concurrentCount }, (_, i) => (0, supertest_1.default)(app)
                .post('/cache/set')
                .send({
                key: `${baseKey}:${i}`,
            ...
            const getPromises = Array.from({ length: concurrentCount }, (_, i) => (0, supertest_1.default)(app).get(`/cache/get/${encodeURIComponent(`${baseKey}:${i}`)}`));
    ...
    describe('Response Schema Validation', () => {
        it('should return consistent response schemas across all endpoints', async () => {
            ...
            const setResponse = await (0, supertest_1.default)(app)
                ...
                .expect(201);
            expect(setResponse.body).toEqual(expect.objectContaining({
                success: expect.any(Boolean),
                key: expect.any(String),
                message: expect.any(String)
            }));
            // Test GET response schema (found)
            const getFoundResponse = await (0, supertest_1.default)(app)
                .get('/cache/get/schema%3Atest')
                .expect(200);
            expect(getFoundResponse.body).toEqual(expect.objectContaining({
                key: expect.any(String),
                value: expect.anything(),
                found: expect.any(Boolean)
            }));
            // Test GET response schema (not found)
            const getNotFoundResponse = await (0, supertest_1.default)(app)
                .get('/cache/get/nonexistent%3Akey')
                .expect(404);
            expect(getNotFoundResponse.body).toEqual(expect.objectContaining({
                key: expect.any(String),
                found: expect.any(Boolean),
                message: expect.any(String)
            }));
            ...
            const mgetResponse = await (0, supertest_1.default)(app)
                ...
                .expect(200);
            expect(mgetResponse.body).toEqual(expect.objectContaining({
            ...
            const bulkSetResponse = await (0, supertest_1.default)(app)
                ...
                .expect(200);
            expect(bulkSetResponse.body).toEqual(expect.objectContaining({
            ...
            const tagInvalidateResponse = await (0, supertest_1.default)(app)
                .post('/cache/invalidate/tag')
                .send({ tag: 'test-tag' })
                .expect(200);
            expect(tagInvalidateResponse.body).toEqual(expect.objectContaining({
                tag: expect.any(String),
                invalidatedCount: expect.any(Number),
                invalidatedKeys: expect.any(Array)
            }));
            // Test PATTERN INVALIDATE response schema
            const patternInvalidateResponse = await (0, supertest_1.default)(app)
                .post('/cache/invalidate/pattern')
                .send({ pattern: 'test:*' })
                .expect(200);
            expect(patternInvalidateResponse.body).toEqual(expect.objectContaining({
...
//# sourceMappingURL=cache-controller.integration.test.js.map